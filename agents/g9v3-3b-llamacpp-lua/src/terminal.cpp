...
void Terminal::banner(std::string_view model,const std::filesystem::path&w,GuardrailProfile p,const SandboxCapabilities&s,bool verbose){std::cout<<paint("1;36","G9 Local Coding Agent")<<"\n";if(verbose)std::cout<<"  model      "<<model<<"\n  workspace  "<<w.string()<<"\n  policy     "<<to_string(p)<<"\n  sandbox    "<<s.backend<<(s.filesystem&&s.network?" (fs+network)":" (limited)")<<"\n";std::cout<<"\n";}
